...
# function for scraping web for weather actual information
def get_weather_data(location):
    ...
    html = session.get(url, params= {'hl': 'en'}) # requesting data with english search
    # Getting the soup from google search
    soup = bs(html.text, 'html.parser')
    # Getting my information from the soup
...
info_col = sg.Column([[sg.Text('', key= '-LOCATION-', font='Calibri 30', background_color= '#FF0000',text_color= '#FFFFFF', pad= 0, visible= False)],
...
if __name__ == '__main__':
    while True:
        ...
        if event == 'Enter':
            ...
            # Showing image depending on the data accustomed from the soup
            if weather in ('Sun', 'Sunny', 'Clear', 'Clear with periodic clouds'):
                window['-IMAGE-'].update('graphics/sun.png')
            if weather in ('Partly Sunny', 'Mostly Sunny', 'Partly cloudy', 'Mostly cloudy', 'Cloudy','Overcast'):
                    window['-IMAGE-'].update('graphics/part sun.png')
            if weather in ('Rain', 'Chance of Rain', 'Ligth Rain','Showers','Scattered Showers','Rain and Snow','Hail',):
                window['-IMAGE-'].update('graphics/rain.png')
            if weather in ('Scattered Thunderstorms','Chance of Storm','Storm','Storm', 'Thunderstorm','Chance of Storm'):
                window['-IMAGE-'].update('graphics/thunder.png')
            if weather in ('Mist','Dust','Fog','Smoke','Haze','Flurries'):
                window['-IMAGE-'].update('graphics/fog.png')
            if weather in ('Freezing Drizzle','Chance of Snow','Sleet','Snow','Icy','Snow Showers'):
                window['-IMAGE-'].update('graphics/snow.png')
    window.close()
