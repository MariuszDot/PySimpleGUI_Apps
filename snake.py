...
# Function for converting position to pixels for simple adjusting of play area
def convert_pos_to_pixel(CELL):
    tl = (int(CELL[0] * CELL_SIZE), int(CELL[1] * CELL_SIZE))
    br = (int(tl[0] + CELL_SIZE), int(tl[1] + CELL_SIZE))
    return tl,br
# Function to positioning an apple in different places
def place_apple():
...
if __name__ == '__main__':
    while True:
        ...
        if time_since_start >= 0.5:
            ...
            #check if snake went out of the play arena
            if not snake_body[0][0] <= CELL_NUM - 1 or \
